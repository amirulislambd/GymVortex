    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-8 bg-[#131313] text-[#e5e2e1]">
      ...
      <div className="bg-[#1c1b1b] border border-[#caf300]/10 p-4 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="font-mono text-xs tracking-wider space-y-1">
          <p className="text-[#caf300] font-bold">
            &gt; SESSION_ID: FORUM_MGR_7749
          </p>
          <p className="text-[#8f9378]">
            TOTAL FOUND: {meta.totalPosts} | PAGE: {page} OF {meta.totalPages}
          </p>
        </div>

        <button
          onClick={() => router.push("/dashboard/trainer/add-post")}
      ...
      {isLoading ? (
        ...
        <div className="space-y-4 mb-8">
          {posts.map((post) => (
            <div
              key={post._id}
              className="bg-[#1c1b1b] border border-[#444932]/20 p-4 flex flex-col md:flex-row justify-between gap-6"
            >
              <div className="flex flex-col sm:flex-row items-start gap-4">
                <div className="w-full sm:w-44 h-26 bg-[#201f1f] relative rounded-sm overflow-hidden flex-shrink-0">
                  ...
                  />
                  <span className="absolute top-2 left-2 font-mono text-[9px] bg-[#caf300] text-[#131313] px-1.5 py-0.5 rounded-[2px] uppercase font-bold">
                    {post.status}
                  </span>
                </div>
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-[#c5c9ac] line-clamp-2 max-w-2xl font-light">
                    {post.description}
                  </p>
                  <div className="flex items-center gap-6 pt-2 font-mono text-xs text-[#8f9378]">
                    <span className="flex items-center gap-1">
                      <Heart className="size-4" /> {post.likes?.length || 0}
                    </span>
                    <span className="flex items-center gap-1">
                      <Comment className="size-4" /> {post.commentsCount || 0}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex md:flex-col gap-2 justify-center">
                ...
                <button
                  ...
                  className="w-full md:px-3 py-1.5 border border-red-500/30 text-red-400 font-mono text-[10px] font-bold uppercase rounded-sm cursor-pointer hover:bg-red-500/30 hover:text-white transition-all"
                >
                  Delete
                </button>
              </div>
      ...
      {meta.totalPages > 1 && (
        <div className="flex justify-center items-center gap-4 pt-4 border-t border-[#444932]/20 font-mono text-xs">
          ...
          <span className="text-white tracking-widest">
            PAGE <span className="text-[#caf300] font-bold">{page}</span> OF{" "}
            {meta.totalPages}
          </span>
