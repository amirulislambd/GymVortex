...
export default function AllClasses() {
  ...
  useEffect(() => {
    ...
    const loadClassesData = async () => {
      try {
        const res = await GetClasses({ page: currentPage, limit: limitPerPage, search: debouncedSearch, category: activeCategory });
        if (ignore) return;
        if (res?.success) {
          ...
          <AnimatePresence mode="wait">
            {loading ? (
              <motion.div 
                ...
                className="flex flex-col items-center justify-center py-32 w-full gap-4 absolute inset-0"
              >
                <ImSpinner9 className="size-10 text-[#caf300] animate-spin" />
                <p className="font-mono text-xs text-[#caf300] tracking-widest uppercase">
                  Synchronizing Class Ledger...
                </p>
              </motion.div>
            ) : classes.length === 0 ? (
              <motion.div 
                ...
                exit={{ opacity: 0, y: -10 }}
                className="text-center py-32 border border border-dashed border-[#444932]/30 rounded-sm w-full"
              >
              ...
              {Array.from({ length: totalPages }, (_, index) => {
                ...
                if (
                  ...
                  if (pageNum === 2 && currentPage > 3) {
                    return <span key="dots-1" className="text-[#444932] px-1">...</span>;
                  }
                  if (pageNum === totalPages - 1 && currentPage < totalPages - 2) {
                    return <span key="dots-2" className="text-[#444932] px-1">...</span>;
                  }
                  return null;
                }
