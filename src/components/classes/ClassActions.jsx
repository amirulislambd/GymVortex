...
import toast from "react-hot-toast"; // <Toaster> import বাদ — root layout এ একটাই থাকবে
...
export default function ClassActions({
  ...
  const handleToggleFavorite = async () => {
    ...
    // Already favorited — button disabled হওয়া উচিত, কিন্তু safety check
    if (isFavorite) {
      toast.error("Already saved to favorites.");
      return;
    }

    // Loading চলছে — double click prevent
    ...
    try {
      ...
      if (result?.success || result?.insertedId) {
        // সফল হলে state update + button disable
        setIsFavorite(true);
        toast.success("Added to favorites!");
      } else if (result?.message?.toLowerCase().includes("already")) {
        // Backend থেকে duplicate বললে
        setIsFavorite(true);
  ...
  return (
    // <Toaster /> নেই — root layout এ একটাই থাকবে
    <div className="bg-[#262626]/70 backdrop-blur-xl border border-[#caf300]/10 p-4 sm:p-6 md:p-8 border-t-4 border-t-[#caf300] rounded-sm">
      ...
      <div className="flex flex-col gap-3">
        ...
        {/* Add to Favorites */}
        <button
          disabled={favoriteLoading || isFavorite || isRestricted}
