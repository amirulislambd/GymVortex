...
// Animation configuration for smooth list items
const listAnimation = {
...
export default function UsersTable({ users, currentPage, totalPages, onToggleBlock, onMakeAdmin }) {
  const router = useRouter();
  const currentSearch = useSearchParams().get("search") || "";

  const handlePageChange = (targetPage) => {
    router.push(`/dashboard/admin/manage-users?search=${currentSearch}&page=${targetPage}`);
  };
    ...
    <div className="bg-zinc-950 border border-zinc-900 rounded-sm p-4 mt-6">
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <motion.tbody 
            layout 
            className="divide-y divide-zinc-900 text-sm"
          >
            <AnimatePresence mode="popLayout">
              {users?.map((user) => (
                ...
                >
                  ...
                  <td className={`py-4 px-4 font-medium ${user.banned ? "text-zinc-600 line-through" : "text-zinc-200"}`}>
                    {user.name}
                  </td>
                  <td className={`py-4 px-4 ${user.banned ? "text-zinc-600" : "text-zinc-400"}`}>
                    {user.email}
                  </td>
                  <td className="py-4 px-4">
                    <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded-xs uppercase tracking-wide ${
                      user.role === "admin" 
                  ...
                  <td className="py-4 px-4 font-bold text-xs uppercase tracking-wider">
                    {user.banned ? (
                      <span className="text-rose-500 flex items-center gap-1.5">
                        <span className="h-1.5 w-1.5 rounded-full bg-rose-500 animate-pulse"></span> Blocked
                      </span>
                    ) : (
                      <span className="text-lime-500 flex items-center gap-1.5">
                        <span className="h-1.5 w-1.5 rounded-full bg-lime-500 animate-pulse"></span> Active
                      </span>
                    )}
                  </td>
                  <td className="py-4 px-4 text-right space-x-2 whitespace-nowrap">
                    ...
                    <button
                      onClick={() => onToggleBlock(user._id, !user.banned)}
      ...
      {/* 📱 MOBILE VIEW */}
      <motion.div layout className="block md:hidden space-y-4">
        <AnimatePresence mode="popLayout">
          {users?.map((user) => (
            <motion.div 
              layout
              ...
              exit="exit"
              key={user._id} 
              className={`p-4 rounded-sm border bg-zinc-950/60 transition-all flex flex-col gap-3 ${
                user.banned ? "border-rose-950/40 bg-rose-950/5" : "border-zinc-900"
              }`}
            >
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <h4 className={`text-sm font-bold truncate ${user.banned ? "text-zinc-600 line-through" : "text-zinc-200"}`}>
                    {user.name}
                  </h4>
              ...
              <div className="flex items-center justify-between border-t border-zinc-900 pt-2 mt-1">
                <div className="flex items-center gap-2">
                  <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded-xs uppercase tracking-wide ${
                    user.role === "admin" 
                      ? "bg-red-950 text-red-400 border border-red-900" 
                      : user.role === "trainer"
                      ? "bg-lime-950 text-lime-400 border border-lime-800"
                  ...
                  {user.banned ? (
                    <span className="text-[10px] text-rose-500 font-bold uppercase tracking-wider flex items-center gap-1">
                      <span className="h-1 w-1 rounded-full bg-rose-500 animate-pulse"></span> Blocked
                    </span>
                  ) : (
                    <span className="text-[10px] text-lime-500 font-bold uppercase tracking-wider flex items-center gap-1">
                      <span className="h-1 w-1 rounded-full bg-lime-500 animate-pulse"></span> Active
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1.5">
                  {user.role === "user" && !user.banned && (
                    <button 
                      onClick={() => onMakeAdmin(user._id)}
                      className="border border-lime-500/30 text-lime-400 font-extrabold text-[9px] uppercase tracking-wider px-2 py-1.5 rounded-xs transition-all"
                    >
                      Make Admin
                    </button>
                  )}
                  <button
                    onClick={() => onToggleBlock(user._id, !user.banned)}
                    className={`font-extrabold text-[9px] uppercase tracking-wider px-2 py-1.5 rounded-xs border transition-all ${
                      user.banned
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between border-t border-zinc-900 mt-4 pt-4 gap-3 text-xs text-zinc-500">
        <div>
          Showing <span className="text-zinc-300">{(currentPage - 1) * 10 + 1}-{Math.min(currentPage * 10, totalPages * 10)}</span> of <span className="text-zinc-300">{totalPages * 10}</span> Athletes
        </div>
